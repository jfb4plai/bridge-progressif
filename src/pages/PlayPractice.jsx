 ...
 * Déclarant = Sud, Mort = Nord — le joueur contrôle S ET N (mort)
 */
...
// Ordre de jeu dans une levée (sens horaire en bridge)
const ORDER = ['W', 'N', 'E', 'S']
...
export default function PlayPractice({ profile, onXpGain }) {
  ...
  // Avance la levée : l'IA joue les sièges E et W ; s'arrête sur S et N (joueur)
  const advance = useCallback((newTrick, newHands) => {
    ...
    while (t.length < 4) {
      ...
      // S = déclarant, N = mort → le joueur humain joue les deux
      if (nextSeat === 'S' || nextSeat === 'N') {
        setToPlay(nextSeat)
        break
      }
    ...
    if (t.length === 4) {
      ...
      setTotal(newTotal)
      setMessage(isNS
        ? (lang === 'fr' ? '✓ Levée N-S' : '✓ N-S trick')
        : (lang === 'fr' ? '✗ Levée E-O' : '✗ E-W trick'))
      ...
      setTimeout(() => {
        setMessage(null)
        if (isNS) {
          // N ou S gagne → le joueur entame la prochaine levée
          setTrick([])
          setToPlay(winner)
        } else {
          // E ou W gagne → l'IA entame
  ...
  }, [tricksNS, tricksEW, total, trump, contract, deal, lang, onXpGain, aiPlay])

  // Le joueur joue une carte de Sud
  const handleCardClick = useCallback((card) => {
    if (phase !== 'play' || toPlay !== 'S') return
  ...
  }, [phase, toPlay, legalSud, trick, hands, advance])

  // Le joueur joue une carte du mort (Nord)
  const handleNorthCardClick = useCallback((card) => {
    if (phase !== 'play' || toPlay !== 'N') return
    const key = cardKey(card)
    if (!legalNorth.includes(key)) return

    const newTrick = [...trick, { seat: 'N', card }]
    const newHands = { ...hands, N: hands.N.filter(c => cardKey(c) !== key) }
    advance(newTrick, newHands)
  }, [phase, toPlay, legalNorth, trick, hands, advance])

  // Init : Ouest entame (lead défini dans la donne ou choix IA)
  useEffect(() => {
    const initHands = {
      N: [...deal.north], E: [...deal.east], S: [...deal.south], W: [...deal.west],
    }
    ...
    let lead = westLead
      ? initHands.W.find(c => c.suit === westLead.suit && c.rank === westLead.rank)
      : null

    if (!lead) lead = aiPlay('W', [], initHands)
    if (!lead) return

    const firstTrick = [{ seat: 'W', card: lead }]
    const h0 = { ...initHands, W: initHands.W.filter(c => cardKey(c) !== cardKey(lead)) }
    setHands(h0)
    setTrick(firstTrick)
  ...
  const playMessage = message ?? (
    toPlay === 'N' ? (lang === 'fr' ? 'Jouez le mort (Nord)' : 'Play dummy (North)') :
    toPlay === 'S' ? (lang === 'fr' ? 'Jouez une carte (Sud)' : 'Play a card (South)') :
    ''
  )
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 flex items-center justify-center text-stone-400 text-sm text-center">
          {playMessage}
        </div>
        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-2 text-center">
          <div className="text-xs text-red-700">E-O</div>
          <div className="text-2xl font-bold text-red-700">{tricksEW}</div>
        </div>
      </div>

      {/* Table — deal construit depuis hands (cartes jouées disparaissent) */}
      <BridgeTable
        deal={{ north: hands.N, south: hands.S, east: hands.E, west: hands.W }}
        revealSeats={['S', 'N']}
        currentTrick={trick}
        playableCards={phase === 'play' && toPlay === 'S' ? legalSud : []}
        selectedCard={selected}
        onCardClick={handleCardClick}
        dummyPlayableCards={phase === 'play' && toPlay === 'N' ? legalNorth : []}
        onDummyCardClick={handleNorthCardClick}
        lang={lang}
        declarer="S"
        compact
      />
      ...
      {phase === 'done' && (
        <div className={`rounded-xl border p-4 text-center ${tricksNS >= contract.level + 6 ? 'bg-emerald-50 border-emerald-300' : 'bg-red-50 border-red-300'}`}>
          ...
          <div className="text-sm text-stone-600">
            {lang === 'fr'
              ? `${tricksNS} levées N-S sur ${contract.level + 6} requises`
              : `${tricksNS} N-S tricks, ${contract.level + 6} needed`}
          </div>
          <button
            onClick={() => navigate('/')}
